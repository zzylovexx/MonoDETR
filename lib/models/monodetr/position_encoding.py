...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        if self.normalize:
            ...
            x_embed = x_embed / (x_embed[:, :, -1:] + eps) * self.scale #eps 避免除以0 並把他normalize到0-2pi 因為之後取sin,cos用radius

        dim_t = torch.arange(self.num_pos_feats, dtype=torch.float32, device=x.device)
        dim_t = self.temperature ** (2 * (dim_t // 2) / self.num_pos_feats) #主要posisitonnal 攻式

        pos_x = x_embed[:, :, :, None] / dim_t
        pos_y = y_embed[:, :, :, None] / dim_t #按照pos 並且每個pos 都把posisitonal  embedding 加上來
